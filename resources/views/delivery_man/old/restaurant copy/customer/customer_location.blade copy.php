...
<style>
 ...
 <div class="container">
    ...
    <script>
    const destinationLat = "{{ $delivery_address->latitude }}";
    const destinationLng = "{{ $delivery_address->longitude }}";
    const restaurant_name = "{{ $customer->name }}";
    const destination = `${destinationLat},${destinationLng}`;

    const restaurantIcon = L.icon({
        iconUrl: '{{ $customer->profile_photo_path?$customer->profile_photo_path:asset('restaurant_frontend/no-image.jpg') }}',
        iconSize: [38, 38],
        iconAnchor: [19, 38],
        popupAnchor: [0, -38]
    });
