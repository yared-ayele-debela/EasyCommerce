@foreach($auto_scroll_products as $product)
    ...
    <div class="ecom-product">
        <div class="product-card" data-restaurant-lat="{{ $restLat }}" data-restaurant-lng="{{ $restLng }}"
            data-delivery-radius="{{ $radius }}" data-product-id="{{ $product->id }}">
            {{-- Product Image --}}
            ...
            <div class="action-buttons">
                <button class="action-btn add-to-wishlist" data-product="{{ $product->id }}" aria-label="Add to Wishlist"
                    title="Add to Wishlist">
                    <i class="fas fa-heart"></i>
                </button>

                <button class="action-btn add-to-cart" data-product="{{ $product->id }}"
                    data-product-price="{{ $product->getFinalPrice() }}" aria-label="Add to Cart" title="Add to Cart">
                    <i class="fas fa-shopping-cart"></i>
                </button>

                <button class="action-btn"
                    onclick="window.location.href='{{ url('restaurant/product-detail/' . encrypt($product->id)) }}'"
