                        ...
                        @if($cart && count($cart) > 0)
                            @foreach($cart as $item)
                                @php
                                    ...
                                    $distance = $locationService->getDistance($userLat, $userLng, $restaurant->latitude, $restaurant->longitude);
                                @endphp

                                @if($distance > $restaurant->delivery_radius)
                                    @php $hasOutOfRangeItems = true; @endphp
                                    {{-- <p class="text-danger text-left">
                                        <i class="bi bi-exclamation-triangle-fill"></i>
                                        <strong>Delivery not available for {{ $product->name }}.</strong>
                                    </p> --}}
                                @endif
                            @endforeach

                            @unless($hasOutOfRangeItems)
                            ...
                            @endunless
                            @unless (!$hasOutOfRangeItems)
                            <button type="button" disabled class="checkout-btn text-dark border-0 bg-outline-primary w-100 mt-3">
                                CHECKOUT
                            </button>
                            <p class="text-danger text-left">
                                <i class="bi bi-exclamation-triangle-fill"></i>
                                <strong>Delivery not available for some items in your cart. remove that item to checkout</strong>
                            </p>
                            @endunless
                        @endif
