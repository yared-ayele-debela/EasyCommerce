...
<div class="container">
    ...
    <table class="table">
        ...
        <tr>
            <td>{{ $banner->id }}</td>
            <td><img src="{{ asset('storage/' . $banner->image) }}" loading="lazy" width="100"></td>
            <td>{{ $banner->title }}</td>
            <td>
...
<div class="modal fade" id="createBannerModal" tabindex="-1" aria-labelledby="createBannerModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form action="{{ route('slider-banners.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelectorAll(".editBannerBtn").forEach(button => {
        button.addEventListener("click", function() {
            ...
            document.getElementById("editBannerForm").action = `/admin/restaurant/slider-banners/${id}`;
        });
    });
        ...
        $(document).on("click", ".deleteBannerBtn", function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/admin/restaurant/slider-banners/${id}`,
                        method: "DELETE",
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
