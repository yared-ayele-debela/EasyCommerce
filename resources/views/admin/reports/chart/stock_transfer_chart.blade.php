...
<html>
<head>
    <title>Stock Transfer Chart</title>
    <!-- Include CanvasJS library -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/canvasjs/1.7.0/canvasjs.min.js"></script>
</head>
<body>
    <a href="{{ route('transfer-stock-reports') }}" class="btn btn-primary m-1  ">
    ...
    <script>
        window.onload = function () {
            var chart = new CanvasJS.Chart("chartContainer", {
                ...
                title: {
                    text: "Stock Transfers per Product over Time"
                },
                axisX: {
                    title: "Transfer Date"
                },
                ...
                data: [
                    ...
                    {
                        type: "line",
                        name: "Product {{$data->product_id}} - {{$data->from_warehouse_name}} to {{$data->to_warehouse_name}}",
                        showInLegend: true,
                        dataPoints: [
                            { x: new Date("{{$data->transfer_date}}"), y: {{$data->total_quantity}} }
                        ]
                    },
                    @endforeach
                ]
