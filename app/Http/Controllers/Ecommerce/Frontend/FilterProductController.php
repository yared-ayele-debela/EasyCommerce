...
{
    // Cache each collection separately with tags for easy invalidation
    $groups = Cache::tags(['products', 'groups'])->remember('all_groups', 3600, function () {
        return Group::all();
    });

    $categories = Cache::tags(['products', 'categories'])->remember('all_categories', 3600, function () {
        return Category::all();
    });

    $brands = Cache::tags(['products', 'brands'])->remember('all_brands', 3600, function () {
        return Brand::all();
    });

    $vendors = Cache::tags(['products', 'vendors'])->remember('all_vendors', 3600, function () {
        return Vendor::all();
    });
